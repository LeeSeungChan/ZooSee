...
<div class="BJMainDiv" style="margin-bottom: 10%;">
	<div class="BJPanel" style="width: 80%; margin-left: 10%;">
		...
		<div class="well well-sm"><sec:authentication property="principal.name"/>님이작성하신1:1 질문 리스트</div>
	</div>
			...
			<div class="SBHrAllLine_1">
				<table class="table table-striped table-hover" style="width: 100%;">
					...
					<c:forEach items="${listVO.list}" var="qnaBoard">
						<tr>
							<td>
								<a href="${initParam.root}qna_content.do?boardNo=${qnaBoard.boardNo}">
									${qnaBoard.title}</a>
							</td>
							<td>
								<c:choose>
			...
			<ul class="pagination pagination-sm">
				...
				<c:choose>
					<c:when test="${pb.previousPageGroup}">
						<li><a href="qna_list.do?pageNo=${pb.startPageOfPageGroup-1}">&laquo;</a></li>
					</c:when>
					<c:otherwise>
				...
				<c:forEach var="i" begin="${pb.startPageOfPageGroup}" end="${pb.endPageOfPageGroup}">
					<c:choose>
						<c:when test="${pb.nowPage!=i}">
							<li><a href="qna_list.do?pageNo=${i}">${i}</a></li>
						</c:when>
						<c:otherwise>
				...
				<c:choose>
					<c:when test="${pb.nextPageGroup}">
						<li><a href="qna_list.do?pageNo=${pb.endPageOfPageGroup+1}">&raquo;</a></li>
					</c:when>
					<c:otherwise>
