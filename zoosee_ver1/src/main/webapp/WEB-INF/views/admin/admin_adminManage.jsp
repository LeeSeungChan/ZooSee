				...
				<form id="memberlistForm">
					<table class="table table-striped table-hover" id="memberlistTable">
						...
						<script type="text/javascript">
							...
							location.replace("${initParam.root}interceptor_admin_findById.do");
						</script>
						</c:if>
							<tr>
								<td>${memberVO.id}</td>
								...
								<td>${memberVO.tel}</td>
								<td>${memberVO.rank}</td>
								<td><c:if test="${memberVO.rank=='admin'}">관리자</c:if><c:if test="${memberVO.rank!='admin'}"><input type="button" name="addAdminBtn" value="부여"></c:if></td>
							</tr>
						</c:when>
						...
						<c:forEach items="${list}" var="m">
							<tr>
								...
								<td>${m.tel}</td>
								<td>${m.rank}</td>
								<td><c:if test="${m.rank=='admin'}">관리자</c:if><c:if test="${m.rank!='admin'}"><input type="button" name="addAdminBtn" value="부여"></c:if></td>
							</tr>
						</c:forEach>
						</c:when>
						</c:choose>
