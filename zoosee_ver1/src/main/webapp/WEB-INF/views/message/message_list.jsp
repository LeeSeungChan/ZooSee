	...
	<div class="BJPanel" style="width: 80%; margin-left: 10%;">
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				<sec:authentication property="principal.name"/> 님의 전체 메세지 목록입니다.
	...
	<ul class="pagination pagination-sm">
		...
		<c:choose>
			<c:when test="${pb.previousPageGroup}">
				<li><a href="${initParam.root}message_list.do?pageNo=${pb.startPageOfPageGroup-1}">&laquo;</a></li>
			</c:when>
			<c:otherwise>
		...
		<c:forEach var="i" begin="${pb.startPageOfPageGroup}"
			...
			<c:choose>
				<c:when test="${pb.nowPage!=i}">
					<li><a href="${initParam.root}message_list.do?pageNo=${i}">${i}</a></li>
				</c:when>
				<c:otherwise>
